...
나는 지점이 바로 여기이고, STEP 2에서 사실 정확성을 1순위로 둔 이유다. 전체 품질
평균(2.31/3.00/3.65)만 봤다면 세 모델이 그저 순서대로 벌어져 있다는 것까지만 보였을
텐데, 사례 유형별로 쪼개보고 나서야 그 격차가 정상 문의에서는 거의 없다가 정보 부족
문의에서 8배로 벌어진다는, 실제로 쓸모 있는 차이가 드러났다.
...
품질만 보면 Cloud를 고르는 게 자연스러워 보이지만, 품질·비용·보안 세 축을 나열해두고
나서야 이 서비스에서 실제로 우선인 축이 무엇인지가 분명해졌다 — 여기서는 보안이었고,
그래서 품질이 더 낮은 로컬을 기본값으로 남겼다.
...
## 9. 추가 비교 1 — 베이스 모델과 비교하면 어떨까

STEP 8 완료 후 궁금증에서 시작한 **별도 테스트**다. 발제문의 선택 심화 실습과는
무관하고, 결과가 어느 쪽이든 이미 확정한 Model F 선정은 바뀌지 않는다.

F(`sam-1-base`)는 Qwen2.5-7B-Instruct에 LoRA로 이커머스 특화를 얹은 모델이다.
**그 베이스 모델(G) 자체를 F와 똑같은 조건(질문 10개·2회·temperature 0)으로
돌려보면 파인튜닝이 실제로 효과가 있었는지 드러난다.**

| | F (파인튜닝) | G (베이스) | 차이 |
|---|---|---|---|
| 전체 품질 | 3.65 | **4.15** | G +0.50 |
| C 한국어 표현 | **4.15** | 4.00 | 거의 동일 |
| E 정보 부족 대응 (F의 핵심 강점) | 3.12 | **4.13** | G +1.01 |

**결과가 예상과 반대였다.** 전체 품질·모든 기준·모든 사례 유형에서 **베이스 모델이
F보다 높았다.** 특히 F의 강점으로 꼽았던 "정보 부족 사례에서도 지어내지 않는 능력"이
베이스에서 더 잘 나왔다 — 이 능력이 파인튜닝이 만든 게 아니라 베이스 모델의 일반
instruction tuning에서 왔을 가능성을 보여준다. 한국어 표현도 F·G가 거의 같아서,
"F의 한국어 능력은 English-only 파인튜닝이 아니라 베이스에서 왔다"는 앞선 추정과
맞아떨어진다.

이 결과는 채점 1인·약식 절차라는 한계가 있고, G의 응답이 F보다 1.8배 길어 장황함의
영향도 배제 못 한다. 그래서 여기서 확인한 건 "이 파인튜닝은 효과가 없다"가 아니라
"이 10문항·이 채점 조건에서는 효과가 확인되지 않았다"는 좁은 결론이다 — 문항 대부분이
상세페이지 정보 없이 일반적인 고객 서비스 추론으로 풀리다 보니, 진짜 도메인 지식이
필요한 질문이었다면 결과가 달랐을 수 있다. 전체 채점 근거: [docs/extra-comparison-1-base-vs-finetuned.md](docs/extra-comparison-1-base-vs-finetuned.md)
...
> 위 5개는 필수 산출물이다. STEP 8 이후 별도로 진행한 추가 비교는
> [docs/extra-comparison-1-base-vs-finetuned.md](docs/extra-comparison-1-base-vs-finetuned.md) (9절 참조) — 필수 평가 대상이 아니다.

...
| 중요한 것 | 한국어 성능 · 응답 속도 |
| 중요하지 않은 것 | 데이터 보안 → Cloud를 배제할 이유가 없다는 근거로 STEP 7에서 재사용 |
| 장비 | 노트북 1대 · VRAM 8GB |
...
**운영 권고** — 보안이 중요하지 않다고 정의했으므로 Cloud를 배제할 이유는 없다.
비용과 속도를 기준으로 나눈다.

| 문의 유형 | 처리 위치 | 근거 |
|---|---|---|
| 일반 상품 문의 | **Local F** | F 3.97로 충분, API 과금 0, 응답 3.86s |
| 경계 사례 | **Local F + 검수** | 3.62로 대응 가능하나 오답 시 영향이 크다 |
| 정보 부족 문의 | **Cloud 또는 상담원** | F 3.08 / Cloud 4.00 — F의 최약점 |
| 대량·상시 트래픽 | **Local F** | Cloud는 호출당 과금이 누적된다 |

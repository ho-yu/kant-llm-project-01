...
> 로컬 품질은 각 모델의 **Run 1·Run 2 평균**이다 (`eval-results.md` 에서 나온다).
> **회차 중 좋은 쪽만 고르지 않는다** — 발제가 금지하는 비교 방식이다.
...
- [ ] **로컬의 좋은 결과 한 건만 골라 Cloud 와 비교하지 않는다**
      → 로컬 값은 **Run 1·Run 2 평균**을 쓴다. 회차 중 높은 쪽을 고르면 비교가 성립하지 않는다.
        이 프로젝트는 회차 편차가 실제로 크다 (예: F 의 Q03 Run1 3.00 / Run2 4.00) —
        평균을 쓰되 편차가 큰 문항은 그 사실을 함께 적는다

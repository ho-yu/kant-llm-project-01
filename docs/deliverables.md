...
> 비교 대상 3개 중 이커머스 도메인과 일치하는 후보가 F 하나뿐이라 "F가 이긴다"는
> 방향 자체는 실험 전에도 예상 가능했다. 이 실험이 실제로 검증한 것은 방향이 아니라
> **격차의 크기와 위치** — 정상 사례 9.7%에서 정보 부족 사례 84.4%까지 8배 넘게
> 벌어진다는 사실이며, 이는 도메인 라벨만으로는 알 수 없고 실측으로만 확인할 수 있었다.
> 전체 품질 평균(2.31/3.00/3.65)만 봤다면 순위만 확인하고 끝났을 것이고, 사례
> 유형별로 나눠보고 나서야 실제로 사고가 나는 지점이 어디인지가 드러났다.
...
| 도메인 파인튜닝 효과 | STEP 8 완료 후 별도로 진행한 **추가 비교 1**(F vs 베이스 모델)에서, 전체 품질·모든 기준·모든 사례 유형에서 베이스 모델이 F보다 높게 나왔다. 이 평가셋에서는 도메인 파인튜닝의 이점이 확인되지 않았다 — 상세: [extra-comparison-1-base-vs-finetuned.md](extra-comparison-1-base-vs-finetuned.md) |
...
> 제한하는 조건으로 작용한다.
>
> 품질만 보면 Cloud(4.53)를 고르는 쪽이 자연스러워 보이지만, 품질·비용·보안을
> 나란히 늘어놓고 나서야 이 서비스에서 실제로 우선인 축이 무엇인지가 분명해졌다.
> 여기서는 보안이 품질 차이보다 앞섰고, 그래서 품질이 더 낮은 로컬을 기본값으로 뒀다.
...
발제문의 선택 심화 실습(3번째 로컬 모델 비교, Quantization 비교, Transformers 직접 실행,
Sentence Transformers 임베딩)과는 별개로, STEP 8 완료 이후 궁금증에서 시작해 추가로
진행한 테스트다. 필수 완료 기준·평가표에는 포함되지 않으며, 결과는 STEP 8에서 확정한
Model F 선정을 바꾸지 않는다.

## 추가 비교 1 — 베이스 모델 vs 파인튜닝 모델 (F vs G)

**질문**: F(`sam-1-base`)는 Qwen2.5-7B-Instruct 에 LoRA 로 이커머스 특화를 얹은
모델이다. F의 모델 카드가 English only 를 명시하는데도 한국어 표현 점수가 가장
높았던 것을 계기로, **베이스 모델(Model G) 자체를 F와 동일 조건(질문 10개·반복
2회·temperature 0 등)으로 돌려 파인튜닝의 실제 효과를 확인**했다.

| | F (파인튜닝) | G (베이스) | 차이 |
|---|---|---|---|
| 전체 품질 (n=72) | 3.65 | **4.15** | G +0.50 |
| A 답변 적합성 | 3.70 | **4.50** | G +0.80 |
| B 논리성/실용성 | 3.28 | **3.78** | G +0.50 |
| C 한국어 표현 | **4.15** | 4.00 | 거의 동일 |
| D 지시사항 준수 | 3.67 | **4.67** | G +1.00 |
| E 정보 부족/불확실성 대응 | 3.12 | **4.13** | G +1.01 |

**결과 요약**: 전체 품질·모든 기준·모든 사례 유형(정상/경계/정보 부족)에서 베이스
모델(G)이 F보다 높았다. 특히 F의 핵심 강점이었던 정보 부족 사례 대응(E)에서도
G가 더 높아, **이 평가셋 기준으로는 도메인 파인튜닝의 이점이 확인되지 않았다.**
한국어 표현이 F와 G가 거의 같다는 점도, F의 한국어 능력이 파인튜닝이 아니라
베이스 모델에서 왔다는 근거로 읽힌다.

**단서**: 이 채점은 F 채점과 같은 기준으로 진행했으나 STEP 6 공식 절차(다회
재검토)만큼 엄격하지 않은 약식이다. G의 응답이 F보다 약 1.8배 길어 장황함의
영향도 배제할 수 없다. 이번 10문항 자체가 상세페이지 정보 없이 일반적인 고객
서비스 추론으로 풀리는 문항이 많아, 도메인 지식이 필요한 상황에서는 결과가
달라질 수 있다. 그래서 이 결과는 "도메인 파인튜닝은 효과가 없다"가 아니라
"이 평가 조건에서는 이점이 확인되지 않았다"로 좁혀 읽어야 한다.

전체 채점 근거·해석: [extra-comparison-1-base-vs-finetuned.md](extra-comparison-1-base-vs-finetuned.md)

...
|---|---|
| 수행 형태 | 개인 |
| 사용 사례 | 상품 상세페이지 정보 기반 이커머스 고객 문의·셀러 업무 자동 응답 |
...
> 기준마다 출제 문항 수가 달라 `n` 이 다르다. 품질 평균(n=72)은 이 개별 점수 72개를
> 모두 모아 낸 평균이다. 문항별·사례 유형별·Run 간 일관성 집계는
> [`local_summary.md`](../data/derived/tables/local_summary.md) 에 있다.
...
| **보안** | 데이터·모델을 직접 통제, 내부망 서빙 가능. 접근 제어·패치·로그는 운영자 책임 | 질문이 외부 API 로 전송됨. 개인정보 최소화와 제공자 정책 검토 필요 | 운영 조건 분석 (실측 아님) |
...
|---|---|---|---|
| 상품 정보 기반 정확한 응답 (없는 정보를 지어내지 않기) | 기준 **E** · 선호 우선순위 **1위** | F 3.12 / D 2.25 / C 1.38 (n=8) | **F만 3점대.** 정보 부족 사례에서 C·D 는 나란히 1.67로 실패 |
...
| 문의 유형 | 처리 위치 | 근거 |
|---|---|---|
| 일반 상품 문의 (정상 사례) | **Local F** | F 3.97로 충분, API 과금 0, 응답 3.86s |
| 경계 사례 (책임 소재 불명확 등) | **Local F + 검수** | F 3.62로 대응 가능하나 오답 시 영향이 크다 |
| 정보 부족 사례 (상세페이지에 없는 정보) | **Cloud 또는 상담원** | F 3.08 / Cloud 4.00 — F 의 최약점 구간 |
| 대량·상시 트래픽 | **Local F** | Cloud 는 호출당 과금이 누적된다 |

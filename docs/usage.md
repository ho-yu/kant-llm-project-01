...
```
data/derived/responses/Q01.md   ← Q01 의 6개 모델 × 2회 응답 전문 (채점 대상이 먼저)
...
| 부가 테스트 | A (법률) · B (의료·바이오) · E (수학) | 60 |

두 파일 모두 **비교 대상이 먼저** 나오고, 그 뒤에 부가 테스트가 표시와 함께 붙는다.
부가 테스트는 성능 측정(STEP 6)에는 그대로 들어가지만 품질 채점 진행률과
필수 조건 6 판정에서는 빠진다. 채점하면 부가 항목으로 따로 집계된다.

대상을 바꾸려면 `data/config/models.json` 의 `tier` 를 고친다
(`primary` = 비교 대상, `supplementary` = 부가). 코드는 건드릴 필요가 없다.

120개 블록이 이미 만들어져 있고, 블록 제목이 곧 `run_id` 다.

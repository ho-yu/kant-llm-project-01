...
화면 출력과 같은 내용이 아래 두 파일에도 매번 다시 쓰인다.
복사해 붙이지 않아도 되고, 모델을 하나씩 돌릴 때마다 내용이 누적된다.

| 파일 | 내용 |
|---|---|
| `data/derived/step04.md` | 실행한 모델마다 STEP 04 절이 쌓인다 |
| `data/derived/step06.md` | 실행한 모델 전체가 한 표에 들어간다 |

생성물이므로 직접 수정하지 않는다. 제출할 때 `docs/steps/step04.md` ·
`step06.md` 에 옮기거나, 그 문서에서 이 파일을 가리키면 된다.
